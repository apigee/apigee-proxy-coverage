...
if UserPW == None or \
    ...
    print """Usage: deploy -n [name] (-d [directory name] | -z [zipfile])
              ...
              [-p [base path] -h [apigee API url] -i -v]
    ...
    -v denotes non-seamless deployments
    """
    sys.exit(1)
            ...
            for fileEntry in dirEntry[2]:
                if not fileEntry.endswith('~'):
                    ...
                    en = os.path.join(
                            ...
                            fileEntry)
                    if (debug): print 'Writing %s to %s' % (fn, en)
                    zipout.write(fn, en)
...
if ShouldDeploy:
    ...
    def nonSeamLessDeployments():
        ...
        # Deploy the bundle
        print 'Deploying proxy version %i' % revision
        hdrs = {'Accept': 'application/json'}
        return httpCall('POST',
            ('/v1/organizations/%s/apis/%s/deployments' +
                    '?action=deploy' +
                    '&env=%s' +
                    '&revision=%i' +
    ...
    deployment_type = {
        True: seamLessDeployments,
        False: nonSeamLessDeployments
    }

    resp = deployment_type[seamless]()
    if resp.status != 200 and resp.status != 201:
